...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ modelOptions, selectedDatabase }) => {
  ...
  const modelGroups: ModelGroup[] = [
    {
      source: 'Ollama',
      models: modelOptions.map(option => option.models).flat() // 当前的模型列表都是来自 Ollama
    }
    // 未来可以添加其他来源的模型组
  ];
  ...
  const sendSqlRequest = async (prompt: string) => {
    ...
    try {
      ...
      console.log("source", selectedModel?.source,);
        ...
        <div className="relative w-48">
          <select
            ...
            className={`
              ...
              ${isSelectShaking ? 'animate-shake' : ''}
              focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500
            `}
            disabled={isLoading}
          >
            <option value="" className="text-gray-500">选择模型</option>
            {modelOptions?.map((sourceOption) => (
              sourceOption.models && sourceOption.models.length > 0 ? (
                <optgroup 
                  key={sourceOption.source} 
                  label={sourceOption.source}
                  className="font-semibold"
                >
                  {sourceOption.models.map((model) => (
                    <option 
                      key={`${sourceOption.source}|${model}`} 
                      value={`${sourceOption.source}|${model}`}
